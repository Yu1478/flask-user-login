...
经测试，所有已知的路径遍历绕过手法均无法读取 pages/ 外的文件：

| 攻击手法 | startswith 结果 | 文件存在？ | 最终结果 |
|----------|----------------|-----------|---------|
| `../app.py` | ❌ 拦截 | — | ✅ 安全 |
| `../../etc/passwd` | ❌ 拦截 | — | ✅ 安全 |
| `pages/../../app.py` | ❌ 拦截 | 是但不通过 | ✅ 安全 |
| `pages/../app.py` | ✅ 放行 | ❌ 不存在 | ✅ 安全 |
| `....//etc/passwd` | ✅ 放行 | ❌ 不存在 | ✅ 安全 |
| `..\app.py`（Linux）| ✅ 放行 | ❌ 不存在 | ✅ 安全 |
| `/etc/passwd`（绝对路径）| ❌ 拦截 | — | ✅ 安全 |

经过双重校验（`startswith` + `os.path.exists`），所有路径遍历尝试均无法读取 pages/ 目录外的任意文件。
