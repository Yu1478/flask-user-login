...
| VULN-504 | SSRF — 302 重定向绕过 | CWE-918 | 🔴 严重 | ✅ 已修复 |
| VULN-505 | SSRF — IPv6 mapped IPv4 绕过 | CWE-918 | 🔴 严重 | ✅ 已修复 |
...
在审查修复方案时，发现了两处绕过漏洞，一并修复：

#### 绕过 1：302 重定向绕过（VULN-504）

攻击者控制的外网服务器返回 302 重定向到内网地址，由于 `urllib.request.urlopen()` 默认跟随重定向，且重定向目标未做 IP 检查，可绕过防护：

```
用户提交 url=http://attacker.com/redirect
         ↓
is_internal_ip("attacker.com") → False(外网) → ✅ 通过
         ↓
urlopen 请求 attacker.com → 返回 302 → Location: http://127.0.0.1:5000/
         ↓
urllib 自动跟随重定向 → 请求内网 → 内网页面返回给用户 ❌ 绕过
```

**修复：** 自定义 `HTTPRedirectHandler`，禁用 302/301 重定向跟随：
...
#### 绕过 2：IPv6 mapped IPv4 地址（VULN-505）

`::ffff:127.0.0.1` 是 IPv6 格式的 IPv4 地址（IPv4-mapped IPv6），解析后 IP 字符串为 `::ffff:127.0.0.1`。按 `.` 分割得到 `['::ffff:127', '0', '0', '1']`，`int('::ffff:127')` 抛出异常。

**修复前：** 依赖异常捕获返回 True（侥幸安全，但不严谨）
**修复后：** 显式处理 `::ffff:` 前缀，提取 IPv4 部分再检查
...
③ DNS 解析 → 获取 IP 地址
    ↓
④ IP 检查 → 内网/保留地址 → 拒绝（含 IPv6 mapped IPv4）
    ↓
⑤ 禁用重定向 → 防止 302 绕过
    ↓
⑥ 正常发起请求 → 返回结果
...
| `http://[::ffff:127.0.0.1]:5000/` | 成功访问 | ❌ IPv6 mapped 拦截 | ② IP 检查 |
| `http://[::1]:5000/` | 成功访问 | ❌ IPv6 拦截 | ② IP 检查 |
| 302 重定向到内网地址 | 跟随重定向放行 | ❌ 禁用 302 跟随 | ③ 禁用重定向 |
...
| # | 测试用例 | 预期 | 实际 | 状态 |
|---|---------|------|------|------|
| 1 | `file:///etc/passwd` | 拦截 | ✅ 不支持的协议 | ✅ |
| 2 | `file:///home/user/app.py` | 拦截 | ✅ 不支持的协议 | ✅ |
| 3 | `http://127.0.0.1:5000/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 4 | `http://localhost:5000/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 5 | `http://10.0.0.1/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 6 | `http://192.168.1.1/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 7 | `http://169.254.169.254/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 8 | `http://0.0.0.0/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 9 | `http://172.16.0.1/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 10 | `http://[::ffff:127.0.0.1]:5000/` | 拦截 | ✅ 不允许访问内网 | ✅ |
| 11 | 外网 302 重定向到内网 | 拦截 | ✅ 302 被禁用，返回 302 状态码 | ✅ |
...
| 🔴 P0 | 解析 DNS 并检查目标 IP 是否为内网地址 | VULN-502, VULN-505 |
| 🔴 P0 | 主机名无法解析时默认拒绝 | VULN-503 |
| 🔴 P0 | 禁用 302 重定向跟随 | VULN-504 |
